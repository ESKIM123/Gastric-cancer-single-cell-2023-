...
Plasma_cancer.subset <- Plasma_subset_anno
DimPlot(Plasma_cancer.subset, reduction = 'umap', raster=FALSE, label = TRUE) + ggtitle("Initial")

view(Plasma_cancer.subset@meta.data)

# Subset only Stage III with tumor and recurrence data

Plasma_cancer_III_subset <- subset(x = Plasma_cancer.subset, Type == 'Tumor' & Patient_ID_SPC != FALSE & Stage == 'Stage III' & Outcome_new != 'NA')
Plasma_cancer_III_subset_Nonrecur <- subset(x = Plasma_cancer.subset, Type == 'Tumor' & Patient_ID_SPC != FALSE & Stage == 'Stage III' & Outcome_new == 'Nonrecur')
Plasma_cancer_III_subset_Recur <- subset(x = Plasma_cancer.subset, Type == 'Tumor' & Patient_ID_SPC != FALSE & Stage == 'Stage III' & Outcome_new == 'Recur')


# plot difference
DimPlot(Plasma_cancer_III_subset, reduction = 'umap', group.by = "Outcome_new", raster=FALSE, label = F, cols = c("#6495ED", "lightgrey"))+
  NoAxes()+NoLegend()+ ggtitle("")

DimPlot(Plasma_cancer_III_subset_Nonrecur, reduction = 'umap', group.by = "Outcome_new", raster=FALSE, label = F, cols = "#6495ED")+
  NoAxes()+NoLegend()+ ggtitle("")
DimPlot(Plasma_cancer_III_subset_Recur, reduction = 'umap', group.by = "Outcome_new", raster=FALSE, label = F, cols = "Red")+
  NoAxes()+NoLegend()+ ggtitle("")

Plasma_cancer_III_subset_Nonrecur
Plasma_cancer_III_subset_Recur

# Cell frequency difference ------------------------------------------------------------------------------------

dittoBarPlot(Plasma_cancer_III_subset, "seurat_clusters", group.by = "Outcome_new", scale = "count", main = "")+
...
dittoBarPlot(Plasma_cancer_III_subset, "seurat_clusters", group.by = "Outcome_new", scale = "percent", main = "")+ 
  NoLegend()+ xlab(NULL)+ ylab(NULL)+
  theme(axis.text.x = element_text(face = "italic"), panel.border = element_rect(color = "black", fill = NA, size = 0.2))
...
dittoBarPlot(Plasma_cancer_III_subset_Nonrecur, "seurat_clusters", group.by = "Patient_ID_SPC", scale = "percent", main = "")+ 
  NoLegend()+ xlab(NULL)+ ylab(NULL)+
  scale_y_continuous(expand = c(0, 0))+
  theme(axis.text.x = element_text(face = "italic"), panel.border = element_rect(color = "black", fill = NA, size = 0.2))

dittoBarPlot(Plasma_cancer_III_subset_Recur, "seurat_clusters", group.by = "Patient_ID_SPC", scale = "percent", main = "")+ 
  NoLegend()+ xlab(NULL)+ ylab(NULL)+
  scale_y_continuous(expand = c(0, 0))+
  theme(axis.text.x = element_text(face = "italic"), panel.border = element_rect(color = "black", fill = NA, size = 0.2))
...
md.Plasma_cancer_III_subset<- Plasma_cancer_III_subset@meta.data %>% as.data.table

length(which(md.Plasma_cancer_III_subset$Patient_ID==4 & md.Plasma_cancer_III_subset$Type=='Tumor'& md.Plasma_cancer_III_subset$seurat_clusters==1))

# Vector of sample numbers
sample_numbers <- c(4, 7, 8, 16, 20, 22, 23, 27, 28, 29, 36, 41, 50, 53, 54, 62, 65, 66)

# Vector of cluster names
cluster_vec <- unique(md.Plasma_cancer_III_subset$seurat_clusters)
type_vec <- c("Nonrecur", "Recur")

unique(Plasma_cancer_III_subset@meta.data$Patient_ID_SPC)
...
for (type in type_vec) {
  for (cluster in cluster_vec) {
    for (number in sample_numbers) {
      ...
      subset_length <- length(which(md.Plasma_cancer_III_subset$Patient_ID_SPC == number & md.Plasma_cancer_III_subset$Outcome_new == type & md.Plasma_cancer_III_subset$seurat_clusters == cluster))
      # Calculate the subset fraction
      total_count <- length(which(md.Plasma_cancer_III_subset$Patient_ID_SPC == number & md.Plasma_cancer_III_subset$Outcome_new == type))
...
################################################################################## DEGS #####################################
DEG_Plasma_cancer_III_subset <- Plasma_cancer_III_subset
DEG_Plasma_cancer_III_subset[["Total"]]  <- "All"
...
View(DEG_Plasma_cancer_III_subset@meta.data)
DEG_Plasma_cancer_III_subset$samples <- paste0(DEG_Plasma_cancer_III_subset$Outcome_new, DEG_Plasma_cancer_III_subset$Patient_ID_SPC)

DEG_Plasma_cancer_III_subset
DefaultAssay(object = DEG_Plasma_cancer_III_subset) <- "RNA"

cts <- AggregateExpression(DEG_Plasma_cancer_III_subset, 
                           group.by = "samples",
                           assays = 'RNA',
                           slot = "counts",
                           return.seurat = FALSE)
...
# SCpubr::do_VolcanoPlot(sample = DEG_Plasma_cancer_III_subset, de_genes = condition.diffgenes, n_genes = 15)
...
VlnPlot(DEG_Plasma_cancer_III_subset, features = "", split.by = "Outcome_new", pt.size = 0)
DefaultAssay(object = DEG_Plasma_cancer_III_subset) <- "integrated"
