...
##################################### Recur vs Non-recur : in Myeloid T cells#########################################

Myeloid_subset <- subset(x = immune_annotation, clusters == 'Myeloid cells')

Myeloid_subset <- RunPCA(Myeloid_subset)
Myeloid_subset <- FindNeighbors(Myeloid_subset, reduction = "pca", dims = 1:50)
Myeloid_subset <- FindClusters(Myeloid_subset, resolution = 0.20)
Myeloid_subset <- RunUMAP(Myeloid_subset, reduction = "pca", dims = 1:50)

DimPlot(Myeloid_subset, reduction = 'umap', raster=FALSE, label = TRUE) + ggtitle("Myeloid cells")
DimPlot(Myeloid_subset, reduction = 'umap', split.by = "Outcome", raster=FALSE, label = T) + NoLegend() + ggtitle("Myeloid T cells")


##################################### Stats #########################################
Myeloid_subset_outcome <- subset(x = Myeloid_subset, Type == 'Tumor' & Outcome != 'Meta'& Molecular_type == "GS" & Patient_ID_SPC != "NA")
...
md.Myeloid_subset_outcome<- Myeloid_subset_outcome@meta.data %>% as.data.table

length(which(md.Myeloid_subset_outcome$Patient_ID==68 & md.Myeloid_subset_outcome$Outcome=='Recur'& md.Myeloid_subset_outcome$seurat_clusters==1))

# Vector of sample numbers
sample_numbers <- c(59, 66, 44, 41, 18, 54, 4, 56, 52, 68, 29, 7, 36, 28, 8)

# Vector of cluster names
cluster_vec <- unique(md.Myeloid_subset_outcome$seurat_clusters)
...
for (type in type_vec) {
  for (cluster in cluster_vec) {
    for (number in sample_numbers) {
      ...
      subset_length <- length(which(md.Myeloid_subset_outcome$Patient_ID_SPC == number & md.Myeloid_subset_outcome$Outcome == type & md.Myeloid_subset_outcome$seurat_clusters == cluster))
      # Calculate the subset fraction
      total_count <- length(which(md.Myeloid_subset_outcome$Patient_ID_SPC == number & md.Myeloid_subset_outcome$Outcome == type))
...
grid.arrange(grobs = plot_list, nrow = 1) |
  grid.text("Myeloid cells", y = 0.05)
...
# Arrange the plots in a grid
grid.arrange(grobs = plot_list, nrow = 1) |
  grid.text("Myeloid cells (Total immune)", y = 0.05)
...
DimPlot(Myeloid_subset, reduction = 'umap', raster=FALSE, label = F) + NoLegend()+NoAxes()

SCpubr::do_BarPlot(sample = Myeloid_subset, 
...
DimPlot(Myeloid_subset, reduction = 'umap', raster=FALSE, label = TRUE) + ggtitle("Myeloid cells")|
  FeaturePlot(Myeloid_subset, features = 'MKI67', raster=FALSE)