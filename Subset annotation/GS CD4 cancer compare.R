...
DimPlot(CD4_subset, reduction = 'umap', raster=FALSE, label = TRUE) + ggtitle("Plasma cells")

##################################### Stats #########################################
CD4_subset_outcome <- subset(x = CD4_subset, Type == 'Tumor' & Outcome != 'Meta'& Molecular_type == "GS" & Patient_ID_SPC != "NA")
...
DimPlot(CD4_subset_outcome, reduction = 'umap', raster=FALSE, label = F) + NoLegend()+NoAxes()
DimPlot(CD4_subset_outcome, reduction = 'umap', raster=FALSE, label = TRUE) + ggtitle("CD4 T cells GS")
DimPlot(CD4_subset_outcome, reduction = 'umap', split.by = "Outcome", raster=FALSE, label = T) + NoLegend() + ggtitle("CD4 T cells GS")
