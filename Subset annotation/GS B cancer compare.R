...
##################################### Recur vs Non-recur : in B cells#########################################
B_subset <- subset(x = immune_annotation, clusters == 'B cells')

B_subset <- RunPCA(B_subset)

B_subset <- FindNeighbors(B_subset, reduction = "pca", dims = 1:44)
B_subset <- FindClusters(B_subset, resolution = 0.2)
B_subset <- RunUMAP(B_subset, reduction = "pca", dims = 1:44)

DimPlot(B_subset, reduction = 'umap', raster=FALSE, label = TRUE) + ggtitle("B T cells")
DimPlot(B_subset, reduction = 'umap', split.by = "Outcome", raster=FALSE, label = T) + NoLegend() + ggtitle("B cells") |
  FeaturePlot(B_subset, features = 'CR2', raster=FALSE)


##################################### Stats #########################################
B_subset_outcome <- subset(x = B_subset, Type == 'Tumor' & Outcome != 'Meta'& Molecular_type == "GS" & Patient_ID_SPC != "NA")
...
md.B_subset_outcome<- B_subset_outcome@meta.data %>% as.data.table

length(which(md.B_subset_outcome$Patient_ID==68 & md.B_subset_outcome$Outcome=='Recur'& md.B_subset_outcome$seurat_clusters==1))

# Vector of sample numbers
sample_numbers <- c(59, 66, 44, 41, 18, 54, 4, 56, 52, 68, 29, 7, 36, 28, 8)

# Vector of cluster names
cluster_vec <- unique(md.B_subset_outcome$seurat_clusters)
...
for (type in type_vec) {
  for (cluster in cluster_vec) {
    for (number in sample_numbers) {
      ...
      subset_length <- length(which(md.B_subset_outcome$Patient_ID_SPC == number & md.B_subset_outcome$Outcome == type & md.B_subset_outcome$seurat_clusters == cluster))
      # Calculate the subset fraction
      total_count <- length(which(md.B_subset_outcome$Patient_ID_SPC == number & md.B_subset_outcome$Outcome == type))
...
# Arrange the plots in a grid
grid.arrange(grobs = plot_list, nrow = 1) |
  grid.text("B cells (Total immune)", y = 0.05)
...
DimPlot(B_subset, reduction = 'umap', raster=FALSE, label = F) + NoLegend()+NoAxes()

SCpubr::do_BarPlot(sample = B_subset, 
