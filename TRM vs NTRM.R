...
immune_annotation[["TRM_NTRM"]]  <- NA

cancer_subset_TRM_NTRM <- subset(x = immune_annotation, Type == 'Tumor' & Patient_ID_SPC != FALSE)
...
md.immune_annotation<- cancer_subset_TRM_NTRM@meta.data %>% as.data.table
...
for (type in type_vec) {
  for (cluster in cluster_vec) {
    for (number in sample_numbers) {
      ...
      subset_length <- length(which(md.immune_annotation$Patient_ID_SPC == number & md.immune_annotation$TRM_NTRM == type & md.immune_annotation$clusters == cluster))
      # Calculate the subset fraction
      total_count <- length(which(md.immune_annotation$Patient_ID_SPC == number & md.immune_annotation$TRM_NTRM == type))
