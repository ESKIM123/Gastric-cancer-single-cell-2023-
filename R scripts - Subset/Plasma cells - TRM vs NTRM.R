...
# Get data ----
Plasma_annotation <- readRDS(file="D:/DATA/Gastric cancer/GS_subsets/Plasma_subset.rds")
DimPlot(Plasma_annotation, reduction = 'umap', raster=FALSE, label = TRUE) + ggtitle("Initial")

##################################### wtp53 vs mtp53 #########################################

Plasma_annotation[["clusters"]]  <- Plasma_annotation[["seurat_clusters"]] 
Plasma_annotation[["TRM_NTRM"]]  <- NA

cancer_subset_TRM_NTRM <- subset(x = Plasma_annotation, Type == 'Tumor' & Patient_ID_SPC != FALSE)
...
md.Plasma_annotation<- cancer_subset_TRM_NTRM@meta.data %>% as.data.table

length(which(md.Plasma_annotation$Patient_ID==68 & md.Plasma_annotation$Type=='Tumor'& md.Plasma_annotation$seurat_clusters==1))
...
cluster_vec <- unique(md.Plasma_annotation$clusters)
type_vec <- c("NTRM", "TRM")
...
for (type in type_vec) {
  for (cluster in cluster_vec) {
    for (number in sample_numbers) {
      ...
      subset_length <- length(which(md.Plasma_annotation$Patient_ID_SPC == number & md.Plasma_annotation$TRM_NTRM == type & md.Plasma_annotation$clusters == cluster))
      # Calculate the subset fraction
      total_count <- length(which(md.Plasma_annotation$Patient_ID_SPC == number & md.Plasma_annotation$TRM_NTRM == type))
