...
# Get data ----
CD8_annotation <- readRDS(file="D:/DATA/Gastric cancer/GS_subsets/CD8_subset.rds")
DimPlot(CD8_annotation, reduction = 'umap', raster=FALSE, label = TRUE) + ggtitle("Initial")

##################################### wtp53 vs mtp53 #########################################

CD8_annotation[["clusters"]]  <- CD8_annotation[["seurat_clusters"]] 
CD8_annotation[["TRM_NTRM"]]  <- NA

cancer_subset_TRM_NTRM <- subset(x = CD8_annotation, Type == 'Tumor' & Patient_ID_SPC != FALSE)
...
md.CD8_annotation<- cancer_subset_TRM_NTRM@meta.data %>% as.data.table

length(which(md.CD8_annotation$Patient_ID==68 & md.CD8_annotation$Type=='Tumor'& md.CD8_annotation$seurat_clusters==1))
...
cluster_vec <- unique(md.CD8_annotation$clusters)
type_vec <- c("NTRM", "TRM")
...
for (type in type_vec) {
  for (cluster in cluster_vec) {
    for (number in sample_numbers) {
      ...
      subset_length <- length(which(md.CD8_annotation$Patient_ID_SPC == number & md.CD8_annotation$TRM_NTRM == type & md.CD8_annotation$clusters == cluster))
      # Calculate the subset fraction
      total_count <- length(which(md.CD8_annotation$Patient_ID_SPC == number & md.CD8_annotation$TRM_NTRM == type))
