...
SCpubr::do_BarPlot(sample = cancer_subset_MSIMSS, 
                   group.by = "clusters", 
                   split.by = "MSI_type",
...
SCpubr::do_BarPlot(sample = cancer_MSI_subset, 
                   group.by = "clusters", 
...
md.immune_annotation<- cancer_subset_MSIMSS@meta.data %>% as.data.table
...
unique(cancer_subset_MSIMSS@meta.data$Patient_ID_SPC)
...
for (type in type_vec) {
  for (cluster in cluster_vec) {
    for (number in sample_numbers) {
      ...
      subset_length <- length(which(md.immune_annotation$Patient_ID_SPC == number & md.immune_annotation$MSI_type == type & md.immune_annotation$clusters == cluster))
      # Calculate the subset fraction
      total_count <- length(which(md.immune_annotation$Patient_ID_SPC == number & md.immune_annotation$MSI_type == type))
...
for (cluster in cluster_vec) {
  ...
  normal_df <- subset_df %>% subset(type_vec == "MSI")
  tumor_df <- subset_df %>% subset(type_vec == "MSS")
